...
def preprocess_fuzzy_rating(fuzzy_rating):
    """Normalize fuzzy_rating (1-5) to range 0-1"""
    return (fuzzy_rating - 1) / 4

def analyze_reviews(file_path):
    """Main analysis function"""
    data = pd.read_csv(file_path)
    
    # Preprocess labels and fuzzy ratings
    data['label'] = data['label'].apply(preprocess_label)
    data['fuzzy_rating'] = data['fuzzy_rating'].apply(preprocess_fuzzy_rating)
...
if __name__ == "__main__":
    results = analyze_reviews('r_pl.csv')
    print("\nAnalysis Results:")
